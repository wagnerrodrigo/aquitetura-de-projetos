package br.com.evaldo.model.bo;

import br.com.evaldo.model.dao.OrdenacaoDAO;
import br.com.evaldo.model.dao.PassoDAO;
    ...
    public Ordenacao bubbleSort(int numero) {
        try {
            ...
            for (int i = 0; i < digitos.length; i++) {
                ...
                //Descrevendo o passo
                passos.add(new Passo(null, null, "Inicio da verificacao numero ".concat(String.valueOf(i)).concat("\n------------\n")));
                continua=false;
                ...
                for (int j = 0; j < digitos.length - 1; j++) {
                    if (digitos[j] > digitos[j + 1]) {
                        ...
                        qtdeTrocas++;
                        //Descrevendo o passo                    
                        passos.add(new Passo(new String(antes), new String(digitos), "Trocou-se o digito ".concat(String.valueOf(digitos[j+1])).concat(" pelo ").concat(String.valueOf(digitos[j]))));
                        continua=true;
                    } else {
                        passos.add(new Passo(new String(digitos), new String(digitos), "Nao houve troca pois o numero ".concat(String.valueOf(digitos[j])).concat(" ja eh menor/igual que ").concat(String.valueOf(digitos[j + 1]))));
                    }
                }
            }
            ...
            new PassoDAO().salvarPassos(passos);
